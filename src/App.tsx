...
export default function App() {
  ...
  const [currentTab, setCurrentTab] = useState<'explore' | 'bookings' | 'admin'>('explore');
  const [showAuthModal, setShowAuthModal] = useState(false);
  const [selectedRoom, setSelectedRoom] = useState<{ hotel: Hotel; room: Room } | null>(null);
  ...
  const handleBookSuccess = (newBooking: Booking) => {
    ...
    if (user && user.role === 'GUEST') {
      ...
      setUser({
        ...
        loyaltyTier: user.loyaltyPoints + ptsToEarn > 3000 ? 'Platinum' : 'Gold',
      });
    }
  ...
  const handleAddRoom = (hotelId: string, newRoom: Room) => {
    setHotels((prev) =>
      prev.map((h) => {
        if (h.id === hotelId) {
  ...
  const handleUpdateBookingStatus = (bookingId: string, status: 'UPCOMING' | 'COMPLETED' | 'CANCELLED') => {
    setBookings((prev) =>
      prev.map((b) => {
        if (b.id === bookingId) {
          return { ...b, status };
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 flex flex-col justify-between font-sans selection:bg-amber-100 selection:text-amber-900">
      ...
      <footer className="bg-white border-t border-gray-100 py-8 text-center text-xs text-gray-400 font-light font-display">
        ...
        <p className="mt-1 font-mono text-[10px] text-gray-300">INTELLIGENT RESORT AUTOMATION SUITE</p>
      </footer>
    </div>
  );
}
