...
export default function HotelMap({ hotels }) {
  ...
  const [activeTarget, setActiveTarget] = useState(mapTargets[0]);
  const [mapZoom, setMapZoom] = useState(14);

  // Generate OpenStreetMap custom map URL based on coordinates
  const getMapUrl = (target) => {
        ...
        <div className="relative z-10 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-400 font-light max-w-2xl">
              Inspect nearby locations, live weather parameters, precise
              satellite positioning, and IoT telemetry loops for our elite
          ...
          <div className="flex flex-wrap gap-2.5">
            {mapTargets.map((target) => (
              ...
              >
                <MapPin
                  className={`w-3.5 h-3.5 ${activeTarget.id === target.id ? "text-white" : "text-slate-500"}`}
                />
                {target.id === "resort_bali"
                  ? "Bali"
                  : target.id === "resort_swiss"
                    ? "Zermatt"
                    : "Tokyo"}
              </button>
            ))}
              ...
              <div className="flex items-center gap-3 font-mono text-[10px] text-slate-400">
                ...
                <div className="flex gap-1 border-l border-white/10 pl-2.5">
                  <button
                    ...
                    className="px-1.5 py-0.5 hover:text-amber-300 transition-colors cursor-pointer"
                  >
                    +
                  </button>
                  <button
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-white/5 rounded-2xl p-4 text-center border border-white/5">
                ...
                <span className="text-xs font-semibold text-slate-200 mt-1 block truncate">
                  {activeTarget.weather.split(",")[1] || "Calm"}
                </span>
              </div>
            </div>
            ...
            <div className="bg-[#0a0c16]/80 rounded-xl p-3.5 border border-white/5 space-y-1.5">
              <div className="flex justify-between text-[11px] text-slate-400">
                ...
                <span className="font-semibold text-white font-mono">
                  {activeTarget.elevation}
                </span>
              </div>
              <div className="flex justify-between text-[11px] text-slate-400">
                <span className="font-mono">Airport Transit Distance</span>
                <span
                  className="font-semibold text-amber-400 text-right max-w-[150px] truncate"
                  title={activeTarget.airportDistance}
                >
            ...
            <div className="space-y-3.5 text-xs font-light">
              <div className="flex items-center justify-between p-2.5 bg-white/5 rounded-xl border border-white/5">
                ...
                <span className="font-bold text-white font-mono">
                  {activeTarget.iotStats.poolTemp}
                </span>
              </div>
              ...
              {activeTarget.iotStats.domeStatus && (
                <div className="flex items-center justify-between p-2.5 bg-white/5 rounded-xl border border-white/5">
                  ...
                  <span className="font-bold text-cyan-400 font-mono text-[10px] text-right">
                    {activeTarget.iotStats.domeStatus}
                  </span>
                </div>
              )}
              ...
              <div className="flex items-center justify-between p-2.5 bg-white/5 rounded-xl border border-white/5">
                ...
                <span className="font-bold text-white font-mono">
                  {activeTarget.iotStats.wifiPing}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between p-2.5 bg-white/5 rounded-xl border border-white/5">
                ...
                <span className="font-bold text-green-400 font-mono">
                  {activeTarget.iotStats.conciergeQueue}
                </span>
              </div>
            </div>
          </div>
